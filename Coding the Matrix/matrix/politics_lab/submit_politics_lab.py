...
def submit(parts_string, login, password):   
    ...
    for sid, name, part_tests in parts:
        ...
        if 'DEBUG' in os.environ: print(' (%s)' % prog_out.replace('\n','\\n'))
        ...
        if len(feedback.strip()) > 0:
            if colorize:
                ...
                print(', got feedback: %s' % feedback.strip())
...
if __name__ == '__main__':
    ...
    group.add_argument('--https', dest="protocol", const="https", action="store_const", help=helps[4])
    group.add_argument('--http',  dest="protocol", const="http",  action="store_const", help=helps[5])
